...
class HistoryComp extends React.Component {
  ...
  handleSearch = () => {
    if (this.search.value == "") {
      ...
      let dataSearch = this.props.barang.filter((item) =>
        item.resi.toLowerCase().includes(this.search.value.toLowerCase())
      );
      this.setState({
        openSearch: dataSearch.length > 0 ? true : false,
        dataSearch,
      });
    } else {
      let dataSearch = this.props.barang.filter((item) =>
        item.desc_status.toLowerCase().includes(this.search.value.toLowerCase())
      );
      this.setState({
        openSearch: dataSearch.length > 0 ? true : false,
        dataSearch,
  ...
  printSearch = () => {
    return this.state.dataSearch.map((item, index) => {
      return (
        <DropdownItem
          onClick={() => {
            this.setState({ selectedIndex: index, modal: !this.state.modal });
          }}
        >
          ...
          <Container>
            <Row>
              ...
              <Col md="3">
                ...
                ) : item.idstatus == 3 || item.idstatus == 4 ? (
                  <>
                    <CardText>
                      {item.desc_status}&nbsp;
                ...
                <Card style={{
                  ...
                  <CardBody>
                    <table style={{ width: "80%" }}>
                      ...
                      <tbody>
                        <tr>
                          <td>Nama Pengirim : {item.nama_pengirim}</td>
                          <td>Nama Penerima : {item.nama_penerima}</td>
                        </tr>
                        {/* <tr>
                          <td>Kota Asal : {item.nama_kota_asal}</td>
                          <td>Kota Penerima : {item.nama_kota_penerima}</td>
                        </tr> */}
                        <tr>
                          <td></td>
                        ...
                        <tr>
                          <td></td>
                          <td>No Telphone : {item.telp_penerima}</td>
                        </tr>
                        <tr>
                          <td></td>
                          <td>Jenis Barang : {item.jenis_barang}</td>
                        </tr>
                        <tr>
                          <td></td>
                          <td>Berat Barang : {item.berat_barang}</td>
                        </tr>
                        <tr>
                          <td></td>
                        ...
                        <tr>
                          <td></td>
                          <td>Harga Pengiriman : {item.harga}</td>
                        </tr>
                      </tbody>
                    </table>
                  </CardBody>
        ...
        <Row>
          <Col md="12" style={{ paddingTop: "5%" }}>
                  ...
                  <Container fluid>
                    <Row>
                      ...
                      <Col md="8 mt-2">
                        ...
                        <h5 style={{ color: "rgb(120,176,242)" }}>
                          {this.state.barangTerkirim}
                        </h5>
                      </Col>
                    </Row>
                  </Container>
            ...
            <Container className="mt-5 pb-5">
              <Row>
                <h5>History</h5>
                <Col md="12 mt-3">{this.printHistory()}</Col>
              </Row>
            </Container>
            {/* TUTUP HISTORY */}
          </Col>
